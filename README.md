...
[![Android CI](https://github.com/Deiivid/Clean_Arquitecture_Compose/actions/workflows/ci.yml/badge.svg)](https://github.com/Deiivid/Clean_Arquitecture_Compose/actions/workflows/ci.yml)
...
The app consumes the three catalog resources exposed by the public Rick and Morty API. Each destination owns its state, pagination and retry behavior while the bottom navigation preserves destination state.
...
- Server pagination with duplicate-request protection, append errors and retry.
...
- Automated unit, lint and assembly gates in GitHub Actions.
...
```mermaid
flowchart LR
    APP[":app<br/>composition root"] --> FEATURES[":feature:*<br/>Compose + ViewModels"]
...
|---|---|---|
| `:app` | Application entry point, navigation, theme and runtime configuration | Features, data, domain |
| `:feature:characters` | Character list UI, state and pagination | Domain, model |
| `:feature:locations` | Location list UI, state and pagination | Domain, model |
| `:feature:episodes` | Episode list UI, state and pagination | Domain, model |
...
Failures cross the data boundary as `Result`; they are represented explicitly in UI state and never silently converted into an empty list.
...
| Images | Coil 3.4.0 |
| Testing | JUnit 4, Turbine, Coroutines Test, MockWebServer |
...
```bash
git clone https://github.com/Deiivid/Clean_Arquitecture_Compose.git
cd Clean_Arquitecture_Compose
...
The JVM suite currently verifies credential-header behavior, use-case delegation, initial ViewModel state and character pagination. GitHub Actions runs the same tests plus Android lint and a debug assembly on every push and pull request.
...
This reference focuses on modular architecture, remote catalogs, pagination, configuration and testability. It does not claim offline persistence, search, detail screens, user authentication or production credential storage.
