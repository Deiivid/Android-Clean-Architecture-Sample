...
internal fun DrawScope.drawDeadBiometrics(radius: Float, rotation: Float, phase: Float) {
    val ember = .32f + abs(sin(phase * PI.toFloat())) * .38f
    drawCircle(
        Brush.radialGradient(
...
internal fun DrawScope.drawUnknownBiometrics(
    ...
    counterRotation: Float,
    phase: Float,
    questionTextMeasurer: TextMeasurer
) {
    drawCircle(
        Brush.radialGradient(
    ...
    repeat(4) { ring ->
        repeat(8) { segment ->
            val blink = .38f + .5f * abs(sin((phase + segment * .13f + ring * .19f) * PI.toFloat()))
            drawArcOnRing(
    ...
    repeat(12) { index ->
        ...
        val floatWave = sin((phase + index * .17f) * PI.toFloat() * 2f)
        ...
        val textSizePx = radius * (.16f + floatWave * .012f)
        val textLayout = questionTextMeasurer.measure(
            text = "?",
            style = TextStyle(
                color = questionColor.copy(alpha = (205f + floatWave * 35f) / 255f),
                fontSize = (textSizePx / (density * fontScale)).sp,
                fontWeight = FontWeight.Bold,
                shadow = Shadow(
                    color = questionColor.copy(alpha = .7f),
                    blurRadius = radius * .035f
                )
            )
        )
        drawText(
            ...
            topLeft = Offset(
                ...
                y = point.y - textLayout.size.height / 2f
            )
        )
    }
}
